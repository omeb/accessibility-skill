---
name: wix-a11y-rules-reference
description: Use when you need detailed guidance on specific WCAG 2.2 AA rules beyond the critical rules — including heading structure, live regions, descriptive links, label-in-name, pointer interactions, hover/focus content, media accessibility, responsive reflow, sensory requirements, autocomplete, language attributes, predictable behavior, error handling, page-level structure, target size, dragging alternatives, accessible authentication, redundant entry, consistent help, motion/seizure safety, high contrast mode, and performance for assistive technology.
---

# Accessibility Rules — Complete Reference

Full code examples and guidance for all WCAG 2.2 AA rules beyond the critical rules in the `web-accessibility` skill. This file is loaded on demand for detailed guidance.
